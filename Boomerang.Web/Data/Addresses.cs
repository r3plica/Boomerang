...
namespace Boomerang.Web.Data
{
    class Addresses
    {
        public static int Create(Address pAddress)
        {
            ...
            SQLCmd.Parameters.Add("Invoice", SqlDbType.Bit).Value = pAddress.Invoice;
        ...
        public static Address Parse(SqlDataReader dr)
        {
            ...
            Address.HouseNumber = Convert.ToString(dr["HouseNumber"]);
            Address.Street = Convert.ToString(dr["Street"]);
            Address.Area = Convert.ToString(dr["Area"]);
            Address.Town = Convert.ToString(dr["Town"]);
            Address.County = Convert.ToString(dr["County"]);
            Address.PostCode = Convert.ToString(dr["PostCode"]);
            Address.Invoice = Convert.ToBoolean(dr["Invoice"]);
